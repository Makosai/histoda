...
test('Climate Ingest - Seeding and Incremental Loading', async (t) => {
  ...
  assert.strictEqual(dateChecks.length, 9, 'Should check missing dates for 9 target stations');
...
test('Climate Ingest - Chunking Large Date Ranges', async (t) => {
  ...
  // Paris Charles de Gaulle (first station) span: 2015-01-02 to 2026-06-15
  // Chunk 1: 2015-01-02 to 2020-01-02
  // Chunk 2: 2020-01-03 to 2025-01-03
  // Chunk 3: 2025-01-04 to 2026-06-15
  // Since there are 9 stations in total, we should see multiple chunk fetches per station.
  assert.ok(fetchUrls.length > 9, 'Should make multiple chunked requests across stations');
